...
#load data
df_bismarck_raw <- haven::read_dta('BDH-Bismarck-JEEA-spec1.dta')
...
```{r}
#save plot as png
ggsave("baseline_model.png", width = 10, height = 6, dpi = 300)
```
````
...
```{r}
#save the individual plots as PNG files
ggsave("Heterogeneity_AdultMales.png", plot_deaths_am_pc, width = 8, height = 6)
ggsave("Heterogeneity_AdultFemales.png", plot_deaths_af_pc, width = 8, height = 6)
ggsave("Heterogeneity_Boys.png", plot_deaths_km_pc, width = 8, height = 6)
ggsave("Heterogeneity_Girls.png", plot_deaths_kf_pc, width = 8, height = 6)
```
...
# 
# 'run the baseline model but with different outcome variables: 
# industry specific death rates'
# 
# #
# 
# #loop through the 13 industries, run regression and create plot for each
# for (i in 1:13) {
#   
#   #regression formula
#   formula <- paste("deaths_tot_pc", 
#                   "factor(year_normalized)", 
#                   sep = "~",
#                   collapse = "+")
#   
#   #specify model name
...
#                       collapse = "+")
#   
#   subset_data <- subset(panel_data, occ == i)
# 
#   #run the regression
#   reg <- plm(formula = deaths_tot_pc ~ bluecollar*factor(year_normalized), 
#             data = subset_data, 
#             model = "within")
#   
#   summary(reg)
...
#   
#   #keep only the last 23 values which are the year coefficients
#   coefficients <- coefficients[(length(coefficients)-22):length(coefficients)]
#   
#   #clustered standard errors
#   clustered_sd <- coeftest(reg, 
#                            vcov = vcovHC(reg,
#                                          type="HC0",
#                                          cluster="group"))
#   
...
#   std_errors <- (clustered_sd)[, 2]
#   #keep only last 23
#   std_errors <- std_errors[(length(std_errors)-22):length(std_errors)]
#   
#   #coefficient names are the years 
#   coef_names <- c(1877:1883, 1885:1900)
#   
#   #create data frame with values and names of coefficients
#   coef_df <- data.frame(coefficient_name = coef_names, 
#                         coefficient = coefficients,
#                         standard_error = std_errors)
# 
#   #create row for year 1884 with the coefficient 0
#   new_row_1884 <- data.frame(coefficient_name = 1884, 
#                              coefficient = 0, 
#                              standard_error = 0)
# 
#   #insert the row between 1883 and 1885
#   coef_df <- rbind(coef_df[1:7,], 
#                    new_row_1884, 
#                    coef_df[8:nrow(coef_df),])
#   
#   #create plot
#   plot <- ggplot(data = coef_df,
#                  aes(x = coefficient_name, 
#                      y = coefficient)) + 
#     geom_point() +
#     #95% conf. interval
...
# 
# #create four plots in a grid to show heterogeneous effects
# combined_plot <- grid.arrange(plot_1, 
#                               plot_2, 
#                               plot_3,
#                               plot_4,
#                               plot_5,
